...
static void remote_reg_failed(dd_broker_t *self, zframe_t *sockid, char *cli_name) {
...
static void s_cb_addbr(dd_broker_t *self, zframe_t *sockid, zmsg_t *msg) {
  ...
  zmsg_print(msg);
    ...
    size_t enclen = sizeof(uint64_t) + crypto_box_NONCEBYTES + crypto_box_MACBYTES;
...
static void s_cb_addlcl(dd_broker_t *self, zframe_t *sockid, zmsg_t *msg) {
  ...
  zmsg_print(msg);
    ...
    ten = (ddtenant_t *) zhash_lookup(dd_broker_keys_get_tenkeys(self->keys), hash);
    free(hash);
    if (ten == NULL) {
...
static void s_cb_challok(dd_broker_t *self, zframe_t *sockid, zmsg_t *msg) {
  ...
  zmsg_print(msg);
    ...
    if (strcmp(hash, dd_broker_keys_get_hash(self->keys)) == 0) {
        ...
        // TODO: how to handle cases where some brokers already have names and some autogenerate?
        // Could be bad if:
        ...
        if (streq("auto", client_name)) {
            ...
            dd_info("Autogenerated scope %s (my scope is %s) ", brscope, self->broker_scope);
            free(client_name);
            client_name = brscope;
        }
        ...
        zsock_send(self->rsock, "fbbbsss", sockid, &dd_version, 4, &dd_cmd_regok,
  ...
  zmsg_print(msg);
    ...
    if (self->state == DD_STATE_REGISTERED)
        zsock_send(self->dsock, "bbbssm", &dd_version, 4, &dd_cmd_forward, 4,
                   &cookie, sizeof(cookie), src_string,
                   dst_string, msg);
}

static void dest_invalid_rsock(dd_broker_t *self, zframe_t *sockid, char *src_string,
                               char *dst_string) {
...
static json_object *json_get_stats(dd_broker_t *self) {
    ...
    ht_node = cds_lfht_iter_get_node(&iter);
    while (ht_node != NULL) {
        local_client *localClient = caa_container_of(ht_node, local_client, rev_node);
        char buf[256];
        json_object *strval = json_object_new_string(localClient->prefix_name);
        json_object_object_add(jlocal_obj, zframe_tojson(localClient->sockid, buf), strval);
...
static json_object *json_get_keys(dd_broker_t *self) {
    ...
    ddtenant_t *ten = (ddtenant_t *) zhash_first(dd_broker_keys_get_tenkeys(self->keys));
...
int dd_broker_add_router(dd_broker_t *self, const char *routeruri) {
    if (self->router_bind == NULL) {
        ...
        int retval = asprintf(&new_router_bind, "%s,%s", self->router_bind, routeruri);
        assert(retval != -1);
...
static void s_self_destroy(dd_broker_t **self_p) {
    ...
    if (*self_p) {
        ...
        if (self->rev_lcl_cli_ht) {
            ...
            if (rc != 0)
                dd_error("s_self_destroy, cds_lfht_destroy(self->rev_lcl_cli_ht) ->  * %d", rc);
