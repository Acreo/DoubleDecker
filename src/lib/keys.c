...
void dd_keys_destroy(dd_keys_t **keys) {
  free((*keys)->hash);
  free((*keys)->privkey);
  free((*keys)->pubkey);
  free((*keys)->ddpubkey);
...
void print_ddbrokerkeys(ddbrokerkeys_t *keys) {
  ...
  while (k) {
    ...
    printf("\t name: %s cookie %lu", ten->name, ten->cookie);
