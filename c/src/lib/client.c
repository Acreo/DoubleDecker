...
static void sublist_add(char *topic, char *scope, char active,
                ddclient_t *dd) {
...
static int publish(char *topic, char *message, int mlen, ddclient_t *dd) {
        ...
        retval =
                crypto_box_easy_afternm(dest, message, mlen, dd->nonce, precalck);
...
static int s_ask_registration(zloop_t *loop, int timerid, void *args) {
        ...
        if (dd->state == DD_STATE_UNREG) {
                ...
                zsock_send(dd->socket, "bbs", &dd_version, 4, &dd_cmd_addlcl, 4,
                                k->hash);
        }
        return 0;
}
...
void print_ddkeystate(ddkeystate_t *keys) {
        ...
        printf("DDPublic key: \t%s",
                        sodium_bin2hex(hex, 100, keys->ddpubkey, 32));
