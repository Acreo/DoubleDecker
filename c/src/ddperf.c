...
static int s_interval_send(zloop_t *loop, int timerid, void *args) {
  int i = 0;
  ddclient_t *dd = (ddclient_t *)args;
...
void s_sendmsg(ddclient_t *dd) {
  ...
  if (latency) {
    ...
    dd->notify("ddperfsrv", (char *)&sendt, sizeof(struct timespec), dd);
  } else {
    if (verbose)
      printf("sending message to ddperfsrv, size %d\n", msize);

    dd->notify("ddperfsrv", rndstr, msize, dd);
  }
}
void on_reg_client(void *args) {
  ...
  if (pps == 0) {
    ...
    send_timer = zloop_timer(dd->loop, interval, 0, s_interval_send, dd);
  }
}
...
void on_data_server(char *source, unsigned char *data, int length,
                    void *args) {
  ddclient_t *dd = (ddclient_t *)args;
  struct timespec sendt, recvt;
  if (latency) {
...
void start_server(char *address) {
  ...
  ddclient_t *client = start_ddthread(1, "ddperfsrv", "public", address,
                                      keyfile, on_reg_server, on_discon,
                                      on_data_server, on_pub, on_nodst);
  ...
  int i = 0;
  while (client->state != DD_STATE_EXIT && !zsys_interrupted) {
    sleep(1);
    s_print_throughput();
    if (i == 10) {
      s_print_stats();
...
void start_client(char *address, int message_num, int message_size) {
  ...
  ddclient_t *client = start_ddthread(1, "ddperfcli", "public", address,
                                      keyfile, on_reg_client, on_discon,
                                      on_data_server, on_pub, on_nodst);
  while (client->state != DD_STATE_EXIT && !zsys_interrupted) {
    sleep(1);
  }
